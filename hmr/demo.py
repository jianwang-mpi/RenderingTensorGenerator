...
def visualize(img, renderer, proc_param, joints, verts, cam):
    """
...
def main(img_path, config, renderer):
    sess = tf.Session()
    model = RunModel(config, sess=sess)

    input_img, proc_param, img = preprocess_image(img_path, config)
    ...
    visualize(img, renderer, proc_param, joints[0], verts[0], cams[0])
...
def run():
    ...
    main(config.img_path, config, renderer)
